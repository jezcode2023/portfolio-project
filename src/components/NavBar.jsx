    ...
    <div className="flex justify-between items-center w-full h-20 px-4 text-white bg-black fixed">
      ...
      <ul className="hidden md:flex md:absolute md:left-1/2 md:transform md:-translate-x-1/2">
        {links.map(({ id, link }) => (
          <li
            key={id}
            className="px-4 cursor-pointer capitalize font-medium text-gray-100 text-2xl hover:scale-105 duration-200"
          >
      ...
      {nav && (
        <ul className="flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-black text-gray-500">
          {links.map(({ id, link }) => (
            <li
              key={id}
              className="hover:scale-105 duration-200 px-4 cursor-pointer capitalize py-6 text-4xl"
            >
